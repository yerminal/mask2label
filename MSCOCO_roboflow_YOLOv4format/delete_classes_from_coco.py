...
background_classes = ["bus", "car", "person", "truck", "motorbike"]#
...
for filename in label_direc:
    ...
    for i in lines:
        ...
        if coco_class_list[int(lst[0])] in forbidden_classes:
            write_file = False
            break
        elif coco_class_list[int(lst[0])] in wanted_classes:
            is_there_wanted_class = True
            temp[coco_class_list[int(lst[0])]] += 1
            line = str(classes_model_ids[coco_class_list[int(lst[0])]]) + " " + " ".join(lst[1:])
            wanted_lines.append(line)
        elif coco_class_list[int(lst[0])] in background_classes:
            temp[coco_class_list[int(lst[0])]] += 1
            line = str(classes_model_ids[coco_class_list[int(lst[0])]]) + " " + " ".join(lst[1:])
            wanted_lines.append(line)
    if is_there_wanted_class and write_file and wanted_lines != []:
        if count % 100 == 0:
